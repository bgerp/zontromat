...
defIfNot('GIT_CMD', 'git');

/**
 * Клас 'git_Lib' - Пакет за работа с git 
 *
 *
 * @category  zontromat
 * @package   lob
...
class lib_Git
{
     ...
     */
    public static function getCurrentBranch($repoPath, &$log)
    {
        ...
        $log[] = "[{$repoName}]: Unable to retrieve the current branch";
    ...
    public static function getTags($repoPath, &$log)
    {
        ...
        $repoName = basename($repoPath);
        $log[] = "[{$repoName}]: Failed to retrieve tags";
    ...
    public static function getRemoteUrl($repoPath, &$log)
    {
        ...
        $repoName = basename($repoPath);
        $log[] = "[{$repoName}]: Failed to retrieve the remote URLg";
    ...
    public static function getLastCommit($repoPath, &$log = null)
    {
        ...
        $log[] = "[${repoName}]: Failed to retrieve last comit";
    ...
    public static function checkout($repoPath, &$log, $branch = 'master')
    {
        ...
        $currentBranch = self::getCurrentBranch($repoPath, $log);
        ...
        if (!self::cmdExec($commandFetch, $arrRes, $repoPath)) {
            foreach ($arrRes as $val) {
                $log[] = (!empty($val))?("[${repoName}]: Error on fetch in {$branch} branch:" . $val):'';
            }
        ...
        if (!self::cmdExec($commandCheckOut, $arrRes, $repoPath)) {
            foreach ($arrRes as $val) {
                $log[] = (!empty($val))?("[${repoName}]: Error on checkout in `{$branch}` branch:" . $val):'';
            }
        ...
        $log[] = "[${repoName}]: switched `{$branch}` branch";
    ...
    { 
        ...
        $currBranch = self::getCurrentBranch($repoPath, $log);
        ...
        if (!self::cmdExec($commandFetch, $lines, $repoPath)) {
            foreach ($lines as $val) {
                $log[] = (!empty($val))?("[${repoName}]: Error on fetch: " . $val):'';
            }
        ...
        if (!self::cmdExec($commandMerge, $lines, $repoPath)) {
            foreach ($lines as $val) {
                $log[] = (!empty($val))?("[${repoName}]: Error on merge origin/" . $currBranch .': ' . $val):'';
            }
            
            return false;
        }
        
        $log[] = "[${repoName}]: Repository is updated";
